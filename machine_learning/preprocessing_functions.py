...
def _id_csvs(data_dir: str) -> dict:
    """
...
def _import_csv(csv_dict: dict, index_col: str = None) -> Dict[str: dd.DataFrame]:
    """
...
def _object_to_cat(df,
                   ignore_cols: list = None,
                   override_cols: list = None) -> dd.DataFrame:
    """
...
def csvs_to_parquets(data_dir: str,
                     ignore_cols: list = None,
                     to_datetime_cols: dict = None,
    ...
    _ = gc.collect()
    print(f'csv_dict stores dask.dataframes with the following keys: {csv_dict.keys()}')
    ...
    # define the output path automatically is param:out_path is not valid/provided
    if not out_path or not os.path.exists(out_path):
        out_path = os.path.join(data_dir, 'output_parquets')

    for title, df in csv_dict.items():
        out_path = out_path + f'\\{title}_parquets'
    ...
    print(f'Done! all .parquet files saved @ {out_path}')
    return out_path
