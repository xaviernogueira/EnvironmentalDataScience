...
class CatBoostML(MachineLearningModel):
    ...
    def get_model_outputs(self,
        ...
        out_dict = {'mean_train_r2': np_train_r2s.mean(),
        ...
        out_dict['train_test_gap'] = out_dict['mean_train_r2'] - out_dict['mean_test_r2']
        return out_dict
    ...
    def regression_kfold_evaluation(self, features: pd.DataFrame,
        ...
        for train_index, test_index in kfolds.split(features):
            ...
            y_train, y_test = features.iloc[train_index].to_numpy(), features.iloc[test_index].to_numpy()
        ...
        performance_dict = self.get_model_outputs(train_r2s,
                                                test_r2s,
                                                train_maes,
                                                test_maes)

        self.performance_log.extend({self.run: performance_dict})

        # get the true and predicted values into
        true_vs_predicted = {'true': np.array(true_values),
                            'predicted': np.array(predicted_values)}
        self.true_vs_predicted.extend({self.run: true_vs_predicted})\
    ...
    def train_regressor(self,
        ...
        if regressor_params is not None:
            ...
            self.parameter_log.extend({self.run: regressor_params})
   
        # train a model over a 10-Kfold CV
        if k_fold_evaluation:
            print(f'{evaluation_kfolds} K-Fold performance evaluation...')
            performance_dict = self.regression_kfold_evaluation(features,
                                                            target,
                                                            regressor_params,
                                                            k_folds=evaluation_kfolds)
            print(f'Done! {evaluation_kfolds} K-Fold perfomance metrics: {performance_dict}')
        
        # make final model and fit to the complete training dataset
        cb_model = cb_model = cb.to_regressor(cb.CatBoost(params=regressor_params))

        return cb_model.fit(features, target, verbose=verbose)
